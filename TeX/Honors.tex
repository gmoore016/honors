...
\begin{document}
	...
	$$U_L(B) - U_H(B) > 0 \iff p(B) > \frac{H - C}{L} \label{choicecon}$$ 
	...
	Note that for each student there exists some $B^*$ such that they are indifferent between $H$ and $L$. Suppose there exists some global upper bound on borrowing $\bar{B}$ which is binding for both those on career paths $H$ and $L$. When $\bar{B}$ is increased, as shown above, this will cause some--but not all--students to shift career from $L$ to $H$ as $\bar{B}$ may shift to be greater than $B^*$ for some subset of students.
	...
	First, I attempt to use a Tobit model to forecast values for debt. Thus, I would like my first stage model to be $$\ln(debt)_{it} = \delta_0 + \delta_1 treatment_{it} + \delta_2 year_t + \delta_3 parentIncome_{it} + \vec{\gamma}^T \vec{\mbox{demographics}}_i + \mu_{it}$$ However, projected values from the Tobit using this model would fail, as errors are only guaranteed to be uncorrelated with fitted values under Ordinary Least Squares. Therefore I use a method suggested by \textcite{angrist2009} whereby the fitted Tobit values are used as an instrument for debt themselves. Thus, we calculate a pre-first stage such that $$\ln(\widehat{debt}')_{it} = \alpha_0 + \alpha_1 \widehat{debt}_{it} + \alpha_2 year_t + \alpha_3 parentIncome_{it} + \vec{\upsilon}^T \vec{\mbox{demographics}}_i + \mu_{it}$$ These projected values are then used as the values for debt in the second stage. 
	...
	In this example, the model would be $$Y = \beta_0 + \beta_1 \widehat{\ln(\mbox{debt})}_{it} + \beta_2 \mbox{hasDebt}_{it} \times \widehat{\ln(\mbox{debt})}_{it} + \vec{\xi}^T \vec{\mbox{demographics}}_i + \epsilon_{it}$$ where $Y$ is some outcome variable. There exist a selection of potential outcome variables worth examining.
	
	Our first model will place major choice on the left hand side. I believe this to be the most novel outcome of interest, as datasets have this level of resolution on their observations. Here I can use a random variable logit model to place students into one of six bins: humanities, STEM, Social Science, Professional, Business, and Pre-Grad. The specifications of these bins can be found in the appendix, and we will adjust these bins later as a form of robustness testing. The distribution of students broken down by major is visible in figure \ref{majDist}.
	
	Our second model will be examining students' career outcomes.
	
	Our final model will be directly examining students' final incomes.
	
	\section{Results}
	
	For testing of the first stage, we find that there exists a significant positive impact on debt of being in the treated cohort, as visible in table \ref{firstStage}. This sample was calculated using the observations from 2006--2010. While the result is only significant at the 6\% level, I feel confident with this instrument both because \textcite{lucca2018} find it it do be significant and because the survey-based nature of our data would be expected to widen the standard errors.
